...
public class RegisterActivity extends BaiduMTJActivity implements OnClickListener {
	...
	private EditText reg_username, reg_password,reg_email, reg_repassword, reg_phone, reg_checknum;
		...
		public void handleMessage(android.os.Message msg) {
			...
			case AppConstants.HANDLER_MESSAGE_NORMAL:
				...
				if (pv.isResult()) {
					...
					dialogUtil.showToast(RegisterActivity.this, "发送成功，请注意查收");
					ThreadExecutor.execute(timeRunable);
				} else {
					dialogUtil.showToast(RegisterActivity.this, "失败：手机号已经注册");
				}
				break;
			...
			case AppConstants.HANDLER_HTTPSTATUS_ERROR:
				dialogUtil.showToast(RegisterActivity.this, "发送验证码失败，请稍后重试");
				break;
			}
		...
		case R.id.reg_check:
			...
			if (TextUtils.isEmpty(phone) || phone.length() != 11) {
				...
				if (reg_check.getTag().equals("未验证")) {
					dialogUtil.showToast(this, "正在发送");
					reg_check.setTag("正在验证");
					// 开始验证线程
				}
